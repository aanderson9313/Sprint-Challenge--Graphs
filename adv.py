...
def travelers_path(direction):
    #save route back to unvisited exits
    if direction == 'n':
        return 's'
    elif direction == 's':
        return 'n'
...
while len(visited) < len(world.rooms):
    ...
    if len(path) > 0:
        move = random.randint(0, len(path) -1) #pick index of move 1-4 and push to stack
        paths.push(path[move])
        player.travel(path[move])
        traversal_path.append(path[move])
        print('there are still rooms to explore!')
    else:
        end = paths.pop()
        player.travel(travelers_path(end))
        traversal_path.append(travelers_path(end))
        print('you have reached the end of this path.')
